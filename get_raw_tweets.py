...
class RawTweet:
    ...
    def get_oneday_twitter(self, idate: str, key_word: str):
        ...
        while True:
            ...
            if i > 5e3:
                print("Reach limit of 5000 requests, check it")
                break
            records = []
            ...
            try:
                ...
                records = [[x.id, x.created_at, x.user.id, x.user.screen_name, x.user.followers_count, x.full_text] for
                           x in result_1qst]
                ...
                if int(actual_date[-2:]) != result_1qst[-1].created_at.day:
                    # if dates diff
    ...
    def get_multiple_dates(self, key_words):
        datelist = list(map(lambda x: str(x.date()), pd.date_range(end=today_date, \
                                                                   periods=self.most_recent_days)))[::-1]
        ...
        for iname in names:
            ...
            while time_gap < period:
                ...
                if len(time_line) == 0:
                    ...
                    break
                # from utc to ets
                time_gap = (today_time - time_line[-1].created_at + datetime.timedelta(
                    hours=4)).total_seconds() / 3600 / 24
                last_maxid = time_line[-1].id
                result += [[x.id, x.created_at, x.user.id, x.user.screen_name, x.user.followers_count, x.full_text] for
                           x in time_line]
                #
